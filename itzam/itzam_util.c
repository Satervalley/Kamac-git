...
ITZAM_SHMEM_TYPE itzam_shmem_obtain(const char * name, size_t len, bool * created) {
        ...
        result = OpenFileMappingA(FILE_MAP_ALL_ACCESS, FALSE, (LPCSTR)name);
...
ITZAM_FILE_TYPE itzam_file_create(const char * filename) {
        ...
        do {
            result = CreateFileA((LPCSTR)filename, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_FLAG_RANDOM_ACCESS, NULL);
...
ITZAM_FILE_TYPE itzam_file_open(const char * filename) {
    ...
    #else
        return CreateFileA((LPCSTR)filename, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_RANDOM_ACCESS, NULL);
    #endif
}
...
bool itzam_file_remove(const char * filename) {
    ...
    #else
        return (bool)DeleteFileA((LPCSTR)filename);
    #endif
}
